...
async function main() {
  ...
  const client = new Client({ host: connection.host, port: 5432, database: 'postgres', user: connection.admin_username,
    ...
    application_name: 'hablas-evo-staging-main-access' });
  let locked = false;
  try {
    ...
    for (const kind of Object.keys(roles)) manifest.secret_refs.push(`local-file:.ops-private/secrets/${ref}-main-${kind}.password`);
