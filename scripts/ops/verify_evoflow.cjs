...
async function main() {
  ...
  const secret = path.join(root, '.ops-private/secrets', ref + '-flow-runtime.password');
  ...
  const options = postgresOptions({ POSTGRES_DB_HOST: target.supabase.main_connection.host, POSTGRES_DB_PORT: '5432',
    POSTGRES_DB_DATABASE: 'postgres', POSTGRES_DB_USERNAME: 'hablas_evo_stg_flow.' + ref,
    POSTGRES_DB_PASSWORD: fs.readFileSync(secret, 'utf8'), POSTGRES_DB_SCHEMA: 'hablas_evoflow_staging',
  ...
  try {
    ...
    const [row] = await runner.query(`SELECT current_user AS role, current_schema() AS schema,
      ...
      has_table_privilege(current_user, 'hablas_evoflow_staging.contacts', 'DELETE') AS can_delete,
