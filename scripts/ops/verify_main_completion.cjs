...
async function main() {
  ...
  try {
    ...
    const row = (await client.query(`SELECT current_user AS role, current_schema() AS schema,
      ...
      has_schema_privilege('hablas_evo_prod_main', 'public', 'CREATE') AS runtime_can_ddl,
      has_schema_privilege('hablas_evo_prod_main', 'hablas_evoflow_production', 'USAGE') AS runtime_can_use_evoflow,
      has_table_privilege('hablas_evo_prod_main', (SELECT c.oid FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
        WHERE n.nspname='auth' AND c.relname='users'), 'SELECT') AS runtime_can_read_auth,
      has_table_privilege('hablas_evo_prod_main', 'public.contacts', 'SELECT') AS runtime_can_select,
      has_table_privilege('hablas_evo_prod_main', 'public.contacts', 'INSERT') AS runtime_can_insert,
      has_table_privilege('hablas_evo_prod_main', 'public.contacts', 'UPDATE') AS runtime_can_update,
      has_table_privilege('hablas_evo_prod_main', 'public.contacts', 'DELETE') AS runtime_can_delete,
      COALESCE((SELECT bool_or(has_table_privilege('hablas_evo_prod_main', c.oid, 'INSERT')
        OR has_table_privilege('hablas_evo_prod_main', c.oid, 'UPDATE')
        OR has_table_privilege('hablas_evo_prod_main', c.oid, 'DELETE'))
    ...
    if (row.role !== 'hablas_evo_prod_main_migrator' || row.schema !== 'public' || row.public_tables !== 110
